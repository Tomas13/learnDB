...
public class ScannerSelectionBarcodeActivity extends Activity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSwitchScannersButton.setOnTouchListener((v, event) -> {
            if (event.getAction() == MotionEvent.ACTION_DOWN && mAidcManager.listConnectedBarcodeDevices().size() > 0) {
                scannerSelection(mAidcManager.listConnectedBarcodeDevices());
            }else{
                Snackbar.make(mSwitchScannersButton, "no  Scanner ", Snackbar.LENGTH_SHORT).show();
            }
            return true;
        });
